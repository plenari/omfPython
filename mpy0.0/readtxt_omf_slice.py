...
def readtxt(fpath,loc=0,mag='mx',plane='xy',ratio=1,print_dict=False,header=True,index=True,save=True):
    ...
    #模型形状  
    znodes=int(float(dic['zmax:'])/float(dic['zstepsize:']))
    ynodes=int(float(dic['ymax:'])/float(dic['ystepsize:']))
    xnodes=int(float(dic['xmax:'])/float(dic['xstepsize:']))
    model_shape=np.array([znodes,ynodes,xnodes])#计算形状
    ...
    if plane=='zx':
        slice_data=data[:,loc,:]
    save_path=fpath.replace('.txt','-'+plane+str(loc)+'-'+mag+'.xlsx')
    if save:
...
if __name__=='__main__':
    ...
    print_dict=False#是否打印从文件头部读取的内容。
    loc=0
    plane='xy'#['xy','yz','zx']
    mag='mz'
    #####保存到excel的设置################################
